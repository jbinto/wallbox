...
const MAP_STYLES = [{"featureType":"all","elementType":"all","stylers":[{"invert_lightness":true},{"saturation":10},{"lightness":30},{"gamma":0.5},{"hue":"#435158"}]}]
...
const getDirectionArrow = ({ location, westPrefix, eastPrefix }) => {
  ...
  if (directionId == null) return QUESTION_MARK
  const isWest = directionId.startsWith(westPrefix)
  const isEast = directionId.startsWith(eastPrefix)
  if (isWest) return LEFT_ARROW
  if (isEast) return RIGHT_ARROW
